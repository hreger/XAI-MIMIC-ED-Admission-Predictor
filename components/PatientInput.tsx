...
export const PatientInput: React.FC<PatientInputProps> = ({ patientData, setPatientData, onPredict, isLoading, isReadOnly }) => {
    ...
    return (
        <Card title="Patient Information" isSticky={true}>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="pt-4">
                    <Button type="submit" disabled={isLoading || isReadOnly} fullWidth>
                        {isLoading ? 'Analyzing...' : isReadOnly ? 'Processing Admission...' : 'Predict Admission'}
                    </Button>
                </div>
            </form>
        </Card>
...
const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement> & { label: string }> = ({ label, ...props }) => (
    <div>
        ...
        <input {...props} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm text-white p-2 disabled:bg-gray-800 disabled:cursor-not-allowed" />
    </div>
);
...
const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement> & { label: string }> = ({ label, children, ...props }) => (
    <div>
        <label className="block text-sm font-medium text-gray-300">{label}</label>
        <select {...props} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm text-white p-2 disabled:bg-gray-800 disabled:cursor-not-allowed">
            {children}
        </select>
    </div>
);
...
const Textarea: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label: string }> = ({ label, ...props }) => (
    <div>
        <label className="block text-sm font-medium text-gray-300">{label}</label>
        <textarea {...props} className="mt-1 block w-full bg-gray-700 border-gray-600 rounded-md shadow-sm focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm text-white p-2 disabled:bg-gray-800 disabled:cursor-not-allowed" />
    </div>
);